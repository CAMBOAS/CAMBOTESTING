...
function buildPrintInvoiceHTML(order, index = 0, total = 1) {
  const province = (order.province || '').trim();
  const detailAddress = (order.address || '').trim();
  const fullAddress = [province, detailAddress].filter(Boolean).join(' : ') || '-';
  const deliveryFee = Math.max(0, Number(order.deliveryFee || 0));
  const itemsTotal = (order.products || []).reduce((sum, item) => sum + calcSubtotal(item), 0);
  const grand = itemsTotal + deliveryFee;
  const grandRiel = Math.round(grand * 4100);
  const rows = (order.products || []).map((it, i) => `
    <tr>
      ...
      <td class="col-qty">${escapeHtml(String(it.qty || 0))} ឈុត</td>
      <td class="col-price">${escapeHtml(formatDisplayMoney(it.price || 0))}</td>
      <td class="col-subtotal">${escapeHtml(formatDisplayMoney(calcSubtotal(it)))}</td>
    </tr>
  `).join('');

  return `
    <section class="print-order ${index < total - 1 ? 'page-break' : ''}">
      <div class="print-head">
        <div>
          <div class="print-title">វិក័យប័ត្រ</div>
          <div class="print-subtitle">Customer Detail Print</div>
        </div>
        <div class="print-meta-right">
          <div><strong>Date:</strong> ${escapeHtml(formatDateForShare(order.date))}</div>
      ...
      <div class="print-info-grid">
        <div><span>ឈ្មោះអតិថិជន</span><strong>${escapeHtml(order.customer || '-')}</strong></div>
        <div><span>លេខទូរសព្ទ</span><strong>${escapeHtml(order.phone || '-')}</strong></div>
        <div><span>ទីតាំង</span><strong>${escapeHtml(fullAddress)}</strong></div>
        <div><span>អ្នកដឹកជញ្ជូន</span><strong>${escapeHtml(order.deliveryName || '-')}</strong></div>
        <div><span>Page</span><strong>${escapeHtml(order.page || '-')}</strong></div>
        <div><span>CloseBy</span><strong>${escapeHtml(order.closeBy || '-')}</strong></div>
        <div><span>Payment</span><strong>${escapeHtml(order.payment || '-')}</strong></div>
        <div><span>Note</span><strong>${escapeHtml(order.note || '-')}</strong></div>
      </div>
      ...
      <table class="print-products-table">
        <thead>
          <tr>
            <th class="col-no">#</th>
            <th class="col-product">ផលិតផល</th>
            <th class="col-qty">ចំនួន</th>
            <th class="col-price">តម្លៃ</th>
            <th class="col-subtotal">សរុប</th>
          </tr>
        </thead>
      ...
      <div class="print-totals">
        <div><span>តម្លៃទំនិញ</span><strong>${escapeHtml(formatDisplayMoney(itemsTotal))}</strong></div>
        <div><span>សេវាដឹក</span><strong>${deliveryFee === 0 ? 'ហ្វ្រីដឹក' : escapeHtml(formatDisplayMoney(deliveryFee))}</strong></div>
        <div class="grand"><span>តម្លៃសរុប</span><strong>${escapeHtml(formatDisplayMoney(grand))}</strong></div>
        <div><span>ប្រាក់រៀល</span><strong>${escapeHtml(grandRiel.toLocaleString())}៛</strong></div>
      </div>
    </section>
  `;
}
...
function getPrintDocumentStyles() {
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .print-title { font-size: 28px; font-weight: 800; line-height: 1; }
    .print-subtitle { font-size: 12px; color: #64748b; margin-top: 4px; }
    .print-meta-right { font-size: 12px; line-height: 1.6; text-align: right; }
    .print-info-grid {
      display: grid;
    ...
    .print-products-table thead th {
      background: #eff6ff;
      font-weight: 800;
    }
    .col-no { width: 42px; text-align: center; }
    .col-product { width: auto; text-align: left; }
    .col-qty, .col-price, .col-subtotal { width: 110px; text-align: right; white-space: nowrap; }
    .print-totals {
      margin-left: auto;
      width: min(100%, 320px);
      display: grid;
    ...
    @media screen {
      body { padding: 16px; background: #e2e8f0; }
